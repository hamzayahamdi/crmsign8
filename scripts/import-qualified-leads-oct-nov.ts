...
/**
 * Script: Import QUALIFIED Leads ONLY from CSV - October-November 2025
 * 
 * This script:
 * 1. Backs up ALL existing lead data with complete history (notes, calls)
 * 2. Clears ALL old lead data
 * 3. Reads from CSV file (qualified - October Novembre.csv)
 * 4. Imports ONLY leads with status "Qualifié" from the CSV
 ...
 * 7. Sets campaignName to "TikTok - Octobre-Novembre 2025"
 * 8. Preserves complete history for leads that existed before (matched by phone)
 * 9. Imports call history (Called on, Deuxième appel, 3 éme appel) as notes
 * 10. Imports notes from CSV
 * 11. Tracks message sending status (WhatsApp)
 */
...
function normalizePropertyType(type: string): string {
    ...
    if (normalized.includes('appartement') || normalized.includes('apt')) return 'Appartement';
    if (normalized.includes('studio')) return 'Studio';
    if (normalized.includes('bureau') || normalized.includes('plateau')) return 'Bureau';
    if (normalized.includes('magasin') || normalized.includes('local')) return 'Magasin';
...
async function importQualifiedLeadsFromCSV(csvPath: string, backup: LeadBackup[]): Promise<void> {
    console.log('📥 Reading CSV file and importing QUALIFIED leads ONLY...\n');
        ...
        fs.createReadStream(csvPath)
            ...
            .on('end', async () => {
                ...
                for (const record of records) {
                    try {
                        // CSV structure from qualified - October Novembre.csv:
                        ...
                        // ⭐ IMPORTANT: Only import leads with status "Qualifié"
                        ...
                        // Status is always "qualifie" since we're only importing qualified leads
                        const statut = 'qualifie';
                        ...
                        const newLead = await prisma.lead.create({
                            data: {
                                ...
                                source: 'tiktok',
                                priorite: 'haute', // Qualified leads are high priority
                                month: 'Octobre-Novembre',
                                campaignName: 'TikTok - Octobre-Novembre 2025',
                                uploadedAt: new Date(),
                                createdAt: new Date(),
                                updatedAt: new Date(),
                            }
                        ...
                        // Track existing note contents to avoid duplicates
                        const existingNoteContents = new Set<string>();
                        ...
                        if (previousLead && previousLead.notes.length > 0) {
                            ...
                            for (const oldNote of previousLead.notes) {
                                ...
                                existingNoteContents.add(oldNote.content.trim().toLowerCase());
                            }
                        }

                        // Helper function to check if a note already exists
                        const noteExists = (content: string): boolean => {
                            const normalizedContent = content.trim().toLowerCase();
                            return existingNoteContents.has(normalizedContent);
                        };

                        // Add call history as notes (only if not already in previous history)
                        if (calledOn) {
                            const callContent = `📞 Premier appel: ${calledOn}`;
                            if (!noteExists(callContent)) {
                                notesToCreate.push({
                                    content: callContent,
                                    author: architect,
                        ...
                        if (deuxiemeAppel) {
                            const callContent = `📞 Deuxième appel: ${deuxiemeAppel}`;
                            if (!noteExists(callContent)) {
                                notesToCreate.push({
                                    content: callContent,
                                    author: architect,
                        ...
                        if (troisiemeAppel) {
                            const callContent = `📞 Troisième appel: ${troisiemeAppel}`;
                            if (!noteExists(callContent)) {
                                notesToCreate.push({
                                    content: callContent,
                                    author: architect,
                        ...
                        // Add note from CSV (only if not already in previous history)
                        if (note) {
                        ...
                        // Add message sent note (only if not already in previous history)
                        if (messageEnvoyer && messageEnvoyer.toLowerCase() === 'oui') {
                            const messageContent = `✉️ Message WhatsApp envoyé`;
                            if (!noteExists(messageContent)) {
                                notesToCreate.push({
                        ...
                        // Create new notes from CSV (only those that don't already exist)
                ...
                console.log(`\n✅ Import complete!`);
                console.log(`   ✅ Imported: ${imported} QUALIFIED leads`);
                console.log(`   ⏭️  Skipped (non-qualified): ${nonQualified} leads`);
                console.log(`   ⏭️  Skipped (empty/invalid): ${skipped} rows`);
                console.log(`   ❌ Errors: ${errors}`);

                console.log(`\n👥 Qualified Leads per Architect:`);
...
async function showSummary() {
    console.log('📊 Summary of imported qualified leads:\n');
    ...
    console.log(`\n   📝 Leads with history: ${leadsWithNotes.length}`);
    console.log(`   📝 Total notes/calls: ${leadsWithNotes.reduce((sum: number, lead: any) => sum + lead._count.notes, 0)}\n`);
}

async function main() {
    try {
        console.log('🚀 Starting QUALIFIED Leads Import - October-November 2025\n');
        ...
        // Step 3: Import QUALIFIED leads ONLY from CSV
        const csvPath = path.join(process.cwd(), 'qualified - October Novembre.csv');

        if (!fs.existsSync(csvPath)) {
            throw new Error(`CSV file not found: ${csvPath}\n\nPlease make sure the CSV file "qualified - October Novembre.csv" exists in the project root directory.`);
        }

        await importQualifiedLeadsFromCSV(csvPath, backup);
        ...
        console.log('📋 All QUALIFIED leads have been imported with their complete history.\n');
        console.log('📋 Each lead is assigned to their designated architect.\n');
