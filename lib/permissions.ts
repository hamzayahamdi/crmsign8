 ...
 */
export function getVisibleSidebarItems(userRole: string | undefined): SidebarItem[] {
  if (!userRole) return []
  
  const normalizedRole = normalizeRole(userRole)
  return sidebarPermissions.filter(item => item.roles.includes(normalizedRole))
}
