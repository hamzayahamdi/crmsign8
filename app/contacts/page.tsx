...
export default function ContactsPage() {
  ...
  useEffect(() => {
    loadContacts()
  }, [searchQuery, statusFilter, selectedArchitect, hasOpportunitiesFilter])
  ...
  const loadContacts = async () => {
    try {
      ...
      // Filter by opportunities if needed
      let filtered = result.data
      if (hasOpportunitiesFilter !== 'all') {
        filtered = result.data.filter(c => {
  ...
  const hasActiveFilters = statusFilter || selectedArchitect || hasOpportunitiesFilter !== 'all'
              ...
              <div className="glass rounded-xl border border-slate-600/30 shadow-[0_18px_48px_-28px_rgba(59,130,246,0.65)]">
                  ...
                  >
                    ...
                    {hasActiveFilters && (
                      <span className="bg-primary/20 text-primary px-2 py-1 rounded-full text-xs font-medium">
                        {(statusFilter ? 1 : 0) + (selectedArchitect ? 1 : 0) + (hasOpportunitiesFilter !== 'all' ? 1 : 0)} actif
                      </span>
                    )}
                    <ChevronDown
                      className={cn(
                ...
                {isFiltersOpen && (
                  <div className="border-t border-slate-600/30 px-4 py-4 bg-slate-900/40 space-y-4">
                    ...
                    <div>
                      <label className="text-xs font-semibold text-slate-300 uppercase tracking-wider mb-2 block">Opportunités</label>
                      <div className="grid grid-cols-3 gap-2">
                        {[
                          { label: 'Tous', value: 'all' },
                          { label: 'Avec opportunités', value: 'has' },
                          { label: 'Sans opportunités', value: 'no' },
                        ].map((filter) => (
                          <button
                            key={filter.value}
